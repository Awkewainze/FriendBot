...
(["SIGINT", "SIGTERM", "SIGQUIT"] as const).forEach((event) => {
	process.on(event, async (code) => {
		...
		await Promise.all([
			client.destroy(),
			TimedEventService.cancelAllEvents()
		]);
		process.exit(0);
	});
});
...
const uncaughtExceptionObservable = fromEvent(process, "uncaughtException", (error: Error, origin: NodeJS.UncaughtExceptionOrigin) => [error, origin] as const);
...
fromEvent(client, "error", (error: Error) => error)
	.subscribe(error => {
...
const onVoiceStateUpdate = fromEvent(client, "voiceStateUpdate", (oldState: VoiceState, newState: VoiceState) => [oldState, newState] as const);

onVoiceStateUpdate
	.pipe(filter(() => !isExiting))
	.subscribe(async ([oldState, newState]) => {
...
function createChildContainerFromInteraction(interaction: BaseInteraction): DependencyContainer {
	const correlationId = v4();
	const interactionLogger = createPrismaLogger({ guildId: interaction.guildId, userId: interaction.user.id, correlationId });
...
const onInteractionCreate = (fromEvent(client, Events.InteractionCreate) as Observable<BaseInteraction>)
	.pipe(filter(() => !isExiting));

onInteractionCreate
	.pipe(filter(x => x.isCommand()))
	.subscribe((commandInteraction: CommandInteraction) => {
		createChildContainerFromInteraction(commandInteraction)
