...
export class MusicPlayer {
    ...
    public constructor(private options: JoinVoiceChannelOptions & CreateVoiceConnectionOptions) {
    ...
    public get stats(): { count: number; time: number; hTime: string } {
        return {
            ...
            hTime: `${this.queue.runTime + this.currentTrackRemaining} seconds remaining`
        };
    }
    ...
    public async enqueueMany(userId: string, tracks: Array<Track>): Promise<void> {
    ...
    private async autoDisconnect(): Promise<void> {
        this.log.info(`auto disconnect triggered for ${this.voiceConnection.joinConfig.channelId || ""}`);
        await this.destroy();
    ...
    private startTimer(): void {
        ...
        this.leaveTimeout = setTimeout(this.autoDisconnect.bind(this), MusicPlayer.autoDCTime);
    }
    ...
    private async processQueue(force = false): Promise<void> {
        ...
        this.log.silly(`processing queue - VC Status = ${this.voiceConnection.state.status}`);
        ...
        if (
            ...
            this.log.verbose(
                `skipping processing due to state; force=${force}; queueLock=${this.queueLock}; status=${
    ...
    private async playerStateHandler(oldState: AudioPlayerState, newState: AudioPlayerState): Promise<void> {
        this.log.verbose(`player for ${this.channelId} state change ${oldState.status} => ${newState.status}`);
        ...
        if (
            ...
            if (
                [AudioPlayerStatus.Idle, AudioPlayerStatus.Paused, AudioPlayerStatus.AutoPaused].includes(
                    oldState.status
                )
            ) {
            ...
            [AudioPlayerStatus.Idle, AudioPlayerStatus.Paused, AudioPlayerStatus.AutoPaused].includes(newState.status)
        ) {
            // start timer
            this.startTimer();
